...
export class FormValidator {
  ...
  validateFieldsById(ids) {
      ...
      ids.forEach((id) => {
          const field = document.getElementById(id);
          if (!field.value.trim() || (field.type === "number" && field.value < 0)) {
      ...
      // Validar os radios
      isValid = this.validateRadioButtons("flexRadioDefault") && isValid;
...
export class FormValidatorLogin {
  constructor(formId) {
      this.form = document.getElementById(formId);
...
export class Login {
  ...
  async login(email, password) {
      ...
      return await this.solicitacoes.fazerSolicitacao("/", dados, "POST");
  }
...
export class Solicitacoes {
    ...
    async fazerSolicitacao(endpoint, dados = {}, metodo = "POST", formId = null) {
        ...
        try {
            ...
            console.log(`Sending ${metodo} request to ${url} with data:`, opcoes);
